...
if __name__ == "__main__":
    print('CRF训练开始')
    args = sys.argv
    train_data_path = KNLP_PATH + "/knlp/data/bios_clue/train.bios"

    if len(args) > 1:
        train_data_path = args[1]

    CRF_trainer = CRFTrain(data_path=train_data_path)
    CRF_trainer.load_and_train()

    CRF_trainer.save_model(KNLP_PATH + "/knlp/model/crf/ner.pkl")
