...
class HMMTrain:
    ...
    def set_transition_pro(self):
        ...
        #     "E": defaultdict(int),
        # }
        count_dict = dict_construct(self.training_data_path, type='trans')
    ...
    def set_emission_pro(self):
        ...
        #     "E": defaultdict(int),
        # }
        count_dict = dict_construct(self.training_data_path, type='emission')
    ...
    def set_init_state_set(self):
        ...
        #     "E": 0,
        # }
        count_dict = dict_construct(self.training_data_path, type='state')
        for line in self.training_data:
            if not line.strip():
                continue
            line = line.strip().split(delimiter)
...
if __name__ == '__main__':
    ...
    print('HMM训练开始')
    args = sys.argv
    vocab_set_path = KNLP_PATH + '/knlp/data/cluener_public/cluener_vocab.txt'
    training_data_path = KNLP_PATH + '/knlp/data/bios_clue/train.bios'

    state_set_save_path = KNLP_PATH + "/knlp/model/hmm/ner"
    transition_pro_save_path = KNLP_PATH + "/knlp/model/hmm/ner"
    emission_pro_save_path = KNLP_PATH + "/knlp/model/hmm/ner"
    init_state_set_save_path = KNLP_PATH + "/knlp/model/hmm/ner"

    if len(args) > 1:
        vocab_set_path = args[1]
        training_data_path = args[2]

    a = HMMTrain(vocab_set_path=vocab_set_path, training_data_path=training_data_path)
    a.init_variable(vocab_set_path=vocab_set_path, training_data_path=training_data_path)
    a.build_model(state_set_save_path, transition_pro_save_path, emission_pro_save_path, init_state_set_save_path)
