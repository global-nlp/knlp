...
from knlp.seq_labeling.bert.mrc_utils import convert_examples_to_features, read_mrc_ner_examples
...
class MRCNERDataLoader(object):
    ...
    def convert_examples_to_features(self, data_sign="train", ):
        ...
        if data_sign == "train":
            examples = read_mrc_ner_examples(os.path.join(self.data_dir), 'train.mrc')
            self.num_train_instances = len(examples)
        elif data_sign == "dev":
            examples = read_mrc_ner_examples(os.path.join(self.data_dir), 'dev.mrc')
            self.num_dev_instances = len(examples)
        elif data_sign == "test":
            examples = read_mrc_ner_examples(os.path.join(self.data_dir), 'test.mrc')
